...
def clip_boxes(boxes, im_shape, min_size=32):
    """
    Clip boxes to image boundaries, and remove small boxes
    """
    # x1 >= 0
...
def crop(im, bboxes, dim0, dim1, len1, len2, min_size=32):
    new_im = im[dim0:dim0 + len1, dim1:dim1 + len2, :]
    new_bboxes = bboxes.copy()
    new_bboxes[:, 0] -= dim1
    new_bboxes[:, 1] -= dim0
    new_bboxes[:, 2] -= dim1
    new_bboxes[:, 3] -= dim0
    return new_im, clip_boxes(new_bboxes, new_im.shape, min_size)
